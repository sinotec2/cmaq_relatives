...
def maping(mn2,image2,x):
    return [ColorDistance(image2[:],x[k,:]) for k in range(N)] 
def map_min(mn2,ll):
    return [ll[i,:].index(min(ll[i,:])) for i in range(mn2[0]*mn2[1])]
def rms(a):
...
clt = KMeans(algorithm='auto', copy_x=True, init='k-means++', max_iter=300, \
    n_clusters=N, n_init=10, n_jobs=-1, precompute_distances='auto', \
...
mmapR=[ldel.index(min([ColorDistance(x[j,:],x2[i,:]) for j in range(N)])) for i in range(200)]
...
for i in range(mn2[0]):
    for j in range(mn2[1]):
        ...
        if m in categoryx[:]:
            conc[i,j]=(float(categoryx[:].index(m)))**2/N*2
        else:
            conc[i,j]=0
...
delx=(mxx-mnx)/(nxd-1)
dely=(mxy-mny)/(nyd-1)
xx=[mnx+delx*i for i in range(nxd)]
yy=[mny+dely*i for i in range(nyd)]
c0=np.empty([nxd,nyd])
for i in range(nxd):
    for j in range(nyd):
...
delx2=(mxx2-mnx2)/nx2
dely2=(mxy2-mny2)/ny2
xx2=[mnx2+delx2*i for i in range(nx2)]
yy2=[mny2+dely2*i for i in range(ny2)]
cn=np.empty([nx2,ny2])
...
for i in range(nx2):
    for j in range(ny2):
        ...
        for ii in range(max(0,ic-3),min(ic+3,nxd)):
            for jj in range(max(0,jc-3),min(jc+3,nyd)):
